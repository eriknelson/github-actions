...
class JiraWatcherManager {
    ...
    async addRemoteWatcher(watcherEmail, watchersUrl) {
    ...
    async deleteRemoteWatcher(watcherEmail, watchersUrl, token) {
        ...
        return new Promise((resolve, reject) => {
            ...
            axios_1.default.delete(watchersUrl, {
                ...
                .then(response => {
                core.info('deleteRemoteWatcher resolving response');
                core.info(`${response.status}`);
                core.info(`${response.statusText}`);
                resolve({
                    email: watcherEmail,
                    statusCode: response.status,
                    error: null,
                ...
                .catch(err => {
                core.info('deleteRemoteWatcher rejecting response');
                core.info(`${err.response.status}`);
                core.info(`${err.response.statusText}`);
    ...
    async ensureDesiredWatchers() {
        ...
        const mutateWatchersFn = (_watcherEmails, mutationFn) => {
            return async () => {
                ...
                const results = await Promise.allSettled(_watcherEmails.map(email => mutationFn(email, watchersUrl)));
        ...
        if (watchersToAdd.length !== 0) {
            core.info('Running watchers to add since its non-zero');
            await mutateWatchersFn(watchersToAdd, this.addRemoteWatcher)();
        }
        if (watchersToDelete.length !== 0) {
            core.info('Running watchers to delete since its non-zero');
